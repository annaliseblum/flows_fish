...
abline(0,1)
plot(S.Flow$avgSflow[S.Flow$type=="USGS"],S.Flow$AvgPreds[S.Flow$type=="USGS"])
abline(0,1)
plot(S.Flow$avgSflow[S.Flow$type=="UVA"],S.Flow$AvgPreds[S.Flow$type=="UVA"],
     main="CV for 5 UVA sites: Obs vs Pred Seasonal Flow LNSE=0.64")
abline(0,1)
...
NSE(log(S.Flow$AvgPreds[S.Flow$type=="UVA"]),log(S.Flow$avgSflow[S.Flow$type=="UVA"])) #still 0.64, not bad!
