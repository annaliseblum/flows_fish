...
length(unique(aDAYMET$site_no)) #173=53 USGS + 5 UVA + 115 Fish
...
SDAYMET$Nseason[SDAYMET$season=="summer"]<- 1
SDAYMET$Nseason[SDAYMET$season=="fall"]<-2
SDAYMET$Nseason[SDAYMET$season=="winter"]<-3
SDAYMET$Nseason[SDAYMET$season=="spring"]<-4
...
SDAYMET_test[1:100,]; tail(SDAYMET_test) #spot check
...
#### 2- Add monthly Precip, max daily precip and max 3 day precip #### WHAT IS GOING ON WITH FIRST YEAR 12 (oh, winter is same year...)
...
sP3day <- aggregate(sortDAYMETS2$P3day,by=list(sortDAYMETS2$site_no, sortDAYMETS2$Nyear, sortDAYMETS2$season),FUN=max)
names(sP3day)<-c("site_no","Nyear","season","maxP3day")

##max
sP1day <- aggregate(DAYMET$prcp,by=list(DAYMET$site_no, DAYMET$Nyear, DAYMET$season),FUN=max)
names(sP1day)<-c("site_no","Nyear","season","maxP1day")

### merge these datasets together to have a seasonal data set with monthly precip sums, 3 day max precip
sP13day<-merge(sP3day, sP1day,by=c("site_no","Nyear","season"))
...
SDAYMET<-merge(SDAYMET,sPrec,by=c("site_no","Nyear","season"))
save(SDAYMET,file="output/SDAYMET.rdata")

#### 3 - Collapse to annual level for both flow and fish predictions - need to be using "fish" year #### 
#load("output/SDAYMET.rdata")
fSDAYMETM <- dcast(SDAYMET, site_no + Nyear ~ season,value.var = "totprecip") #need to get wide format
names(fSDAYMETM)
names(fSDAYMETM)<-c("site_no","Nyear","Pfall", "Pspring", "Psummer", "Pwinter")

fSDAYMETM2 <- dcast(SDAYMET, site_no + Nyear ~ season,value.var = "avgtmax") #need to get wide format
names(fSDAYMETM2)<-c("site_no","Nyear","MaxTfall", "MaxTspring", "MaxTsummer", "MaxTwinter")

fSDAYMETM3 <- dcast(SDAYMET, site_no + Nyear ~ season,value.var = "avgtmin") #need to get wide format
names(fSDAYMETM3)<-c("site_no","Nyear","MinTfall", "MinTspring", "MinTsummer", "MinTwinter")

sP3day_w <- dcast(SDAYMET, site_no + Nyear ~ season,value.var = "maxP3day") #need to get wide format
names(sP3day_w)<-c("site_no","Nyear","maxP3fall", "maxP3spring", "maxP3summer", "maxP3winter")

sP1day_w <- dcast(SDAYMET, site_no + Nyear ~ season,value.var = "maxP1day") #need to get wide format
names(sP1day_w)<-c("site_no","Nyear","maxP1fall", "maxP1spring", "maxP1summer", "maxP1winter")
